...
- Android 默认使用平衡档：频谱与交互目标 30fps、全屏环境画布 15fps、DPR 上限 0.75、初始渲染尺度 0.72，并用提前唤醒的定时节流 `requestAnimationFrame`。唱片在 WebView 91 中使用约 30 个位置/秒的细粒度步进；禁止退回 48 步/圈之类的低步数动画，否则会产生肉眼可见的周期性跳帧。调整这些参数必须在目标模拟器上同时验证流畅度和 CPU，不能只看其中一项。
...
这里不设置 GitHub Actions 或强制提交门禁，但每次修改仍应按影响范围自觉回归：

- 改 H5 后检查 JS 语法、运行 `python tools/sync_web.py`，并确认 `web/` 与 APK assets 一致。
- 改播放、切歌、进度或 Range 后，在雷电模拟器上用真实专辑验证首次播放、上一首/下一首和拖动进度。
- 改专辑库后同时验证单专辑左右按钮/总览，以及两个真实专辑之间的左右切换。
- 改加载或性能逻辑后验证首屏进入时间、正常播放流畅度和 CPU；测 CPU 前必须重启 App，不能遗留持续 rAF 诊断探针。
- 交付前从根目录运行 `build.bat`，只安装 `output/apk/melodio-debug.apk`，不能只凭截图判断完成。
- 临时截图、日志和性能采样统一放进已忽略的 `local/debug/`，调试结束及时清理；禁止把诊断产物散落在仓库根目录。

这些是人工检查清单，不阻止提交。发现某项异常时应如实记录，不要把新功能可用等同于整包无回归。
...
Android 上自动进入移动档（`?performance=` 可强制）：频谱和交互目标 30fps、全屏环境画布 15fps、CSS 更新降频、DPR 上限 0.75、初始渲染尺度 0.72，并使用提前一帧唤醒的定时节流 rAF 调度。唱片在旧 WebView 上采用约 30 个位置/秒的细粒度步进，不能改成肉眼可见的低步数动画。
